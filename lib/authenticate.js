const authenticate =
	(playServerHand, options, combinations) => (req, _res, next) => {
		const { rps, permissive } = req.query;
		...
		const isPermissive = permissive?.toLowerCase() === "true";

		if (isPermissive && isTie) {
			return next();
		}

		if (combinations[serverHand] === clientHand || (!isPermissive && isTie)) {
